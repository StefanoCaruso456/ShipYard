...
export function buildInitialFactoryStagePlans(options: {
  ...
  return [
    ...
    },
    {
      ...
      summary: `Prepare the delivery handoff and summary for ${options.appSpec.appName}.`,
      status: "planned",
      backlog: [
        createSeedBacklogItem({
          ...
          description: `Summarize deployment setup and release risk for ${options.appSpec.appName}.`,
          instruction: `Prepare the deploy handoff for ${options.appSpec.appName} targeting ${options.appSpec.deployment.provider}. Summarize environment variables, manual setup steps, release risks, and the next operator review needed. When complete, explicitly say "Deployment handoff prepared."`,
          expectedOutcome: "Deployment handoff prepared.",
          storyId: "story-delivery-handoff",
          taskId: "task-deployment-handoff",
          acceptanceCriteria: ["Deployment handoff prepared."],
          completionCriterionIds: ["factory-delivery:deployment-handoff"],
          verificationCriterionIds: [
            "factory-delivery:handoff-evidence",
            "factory-delivery:handoff-artifact"
          ],
          preferredSpecialistAgentTypeId: "repo_tools_dev",
          requiredSpecialistAgentTypeId: "repo_tools_dev",
        ...
        createSeedBacklogItem({
          ...
          instruction: `Create the final delivery summary for ${options.appSpec.appName}. Include what shipped, the repository target ${formatRepositoryLabel(options.appSpec.repository.owner, options.appSpec.repository.name)}, the deployment target ${options.appSpec.deployment.provider}, and the next operator action. When complete, explicitly say "Delivery summary prepared."`,
    ...
    case "react_express_railway":
      return [
        ...
        createSeedBacklogItem({
          ...
          description: `Create the Express and data foundation for ${appSpec.appName}.`,
          instruction: `Implement the Express API, shared contracts, and the first end-to-end data flow for ${appSpec.appName}. Connect the frontend shell to the API with clear developer setup notes. When complete, explicitly say "Core product flow implemented."`,
    ...
    case "nextjs_railway_postgres":
      return [
        ...
        createSeedBacklogItem({
          ...
          description: `Create the Railway-backed data flow for ${appSpec.appName}.`,
          instruction: `Implement the first server and database flow for ${appSpec.appName} using the Railway Postgres target. Add the data model, the first interactive flow, and setup notes for local and hosted environments. When complete, explicitly say "Core product flow implemented."`,
          expectedOutcome: "Core product flow implemented.",
    ...
    default:
      return [
        ...
        createSeedBacklogItem({
          ...
          description: `Create the first Supabase-backed product flow for ${appSpec.appName}.`,
          instruction: `Implement the first end-to-end product flow for ${appSpec.appName} using Supabase. Add the initial data model, auth or session handling when appropriate, and the first interactive workflow. When complete, explicitly say "Core product flow implemented."`,
          expectedOutcome: "Core product flow implemented.",
