...
export function normalizeControlPlaneState(
  ...
  normalized.handoffs = Array.isArray(normalized.handoffs)
    ? normalized.handoffs.map((handoff) => ({
        ...handoff,
        correlationId: handoff.correlationId || correlationId(handoff.entityKind, handoff.entityId),
        ...
        acceptanceCriteria: Array.isArray(handoff.acceptanceCriteria)
          ...
          : [],
        validationTargets: Array.isArray(handoff.validationTargets) ? handoff.validationTargets : []
      }))
    : [];
...
function upsertHandoff(
  ...
  if (existing) {
    ...
    existing.dependencyIds = uniqueStrings([...existing.dependencyIds, ...handoff.dependencyIds]);
