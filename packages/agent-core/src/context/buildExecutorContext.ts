...
export function buildExecutorContext(shared: SharedRoleContext): RoleContextPayload {
    ...
    content: JSON.stringify(
      {
        ...
        phaseExecution: shared.run.phaseExecution
          ...
          : null,
        toolRequest: shared.run.toolRequest,
  ...
  if (shared.relevantFiles.length > 0) {
    sections.push({
      id: "relevant-files",
      ...
      content: shared.relevantFiles
        .map((file) => {
          ...
          if (typeof file.startLine === "number" && typeof file.endLine === "number") {
            header.push(`Range: ${file.startLine}-${file.endLine}`);
          }

          if (file.reason) {
            header.push(`Reason: ${file.reason}`);
          }

          if (file.source) {
            header.push(`Source: ${file.source}`);
          }
  ...
  if (shared.recentToolResults.length > 0) {
    sections.push({
      ...
      format: "json",
      content: JSON.stringify(shared.recentToolResults, null, 2)
    });
  } else {
    omittedSections.push({
      id: "recent-tool-results",
      ...
      precedence: "live execution context",
      source: "run.result.toolResult",
      reason: "No completed tool results are attached to this run."
    });
  }

  if (shared.validationTargets.length > 0) {
    sections.push({
  ...
  if (shared.knownFailures.length > 0) {
    sections.push({
      id: "known-failures",
  ...
  return {
    ...
    sections,
    omittedSections,
    prompt: renderPrompt("executor", sections)
  };
}
